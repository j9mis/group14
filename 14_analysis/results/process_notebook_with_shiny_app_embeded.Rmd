...
Our main goal of this project was to allow people to explore data about incidents in which people lost their lives at the hands of US police. We wanted to learn about who police killed in 2015 and about the locations in which these incidents occured without making inferences or predictions. Through exploring the data, we noted how certain groups of people are killed disproportionately more than others in 2015. The plots and figures detailed above *may* suggest that people of certain age groups, genders, races, and areas are more likely to be killed than others. The dataset only covered one year and contained a littl over 1000 data points so it would be irresponsible to make any strong assumptions or assertions about police killings. Since the topic of race and police is so touchy, we created a shiny application to present data to readers with the hopes that they will gather the own valuable insights.    
Based on the Shiny application, we were able to find a few interesting things. 

## Shiny Application
  
Our shiny application has two main builit in features. The first feature it offers to the user is an interactive map. This map shows the location of every single police killing that had occured in 2015. Each point on the map represents a person that was killed, and clicking on a point will bring up a popup that gives information about the deceased, as well as a few background facts about the area in which the killing occured. This map has an additional feature that gives the user the ability to filter points based on certain criteria. They can find those who were killed that were of a given race, those that has a knife, etc. The user can filter the data that appears on the map by one to three attributes, allowing them to view incidents that fit a combination of different criteria. 
 ...
 Based on this change, we can see a shift in the distributions of deaths that may help us to more accurately understand the true nature of the these incidents.
